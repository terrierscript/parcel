...
class JSConcatPackager extends Packager {
  ...
  markUsedExports(asset) {
    ...
    for (let identifier in asset.cacheData.imports) {
      var _asset$cacheData$impo = _slicedToArray(
        asset.cacheData.imports[identifier],
        2
      );

      let source = _asset$cacheData$impo[0],
  ...
  addBundleLoader(bundleType, parentAsset, dynamic) {
    ...
    return _asyncToGenerator(function*() {
      ...
      var _concat = concat(_this5, ast);

      let output = _concat.code;
      ...
      yield Packager.prototype.write.call(_this5, output);
    })();
  }
}
