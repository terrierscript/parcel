...
module.exports = (packager, ast) => {
  let addedAssets = packager.addedAssets;
  ...
  try {
    for (
      var _iterator = addedAssets[Symbol.iterator](), _step;
  ...
  function findExportModule(id, name) {
    ...
    if (wildcards && name !== 'default') {
      ...
      try {
        for (
          var _iterator2 = wildcards[Symbol.iterator](), _step2;
  ...
  function replaceExportNode(module, originalName, path) {
    var _findExportModule = findExportModule(module.id, originalName);

    let identifier = _findExportModule.identifier,
      name = _findExportModule.name,
      id = _findExportModule.id;
  ...
  function interop(mod, originalName, path, node) {
    ...
    if (mod.cacheData.isCommonJS && originalName === 'default') {
      ...
      if (!path.scope.getBinding(name)) {
        var _path$getStatementPar = path.getStatementParent().insertBefore(
            DEFAULT_INTEROP_TEMPLATE({
              NAME: t.identifier(name),
              MODULE: node
            })
          ),
          _path$getStatementPar2 = _slicedToArray(_path$getStatementPar, 1);

        let decl = _path$getStatementPar2[0];
        ...
        if (t.isObjectPattern(id)) {
          ...
          try {
            for (
              ...
              var _findExportModule2 = findExportModule(
                match[1],
                key.name,
                path
              );

              let identifier = _findExportModule2.identifier;
    ...
    MemberExpression: {
      exit(path) {
        ...
        if (match) {
          ...
          var _findExportModule3 = findExportModule(match[1], name, path);

          let identifier = _findExportModule3.identifier;
